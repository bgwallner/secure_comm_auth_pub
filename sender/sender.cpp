// Copyright 2026 Bo-Göran Wallner
//
...
int receive_symmetric_key(mqd_t mq, const Botan::RSA_PrivateKey& private_key,
    ...
    std::cout << "[Sender] Received encrypted ID + symmetric key + signature + signature size (" << bytes_received << " bytes)\n";
    ...
    // Print received encrypted ID + symmetric key in hex
    std::cout << "[Sender] Received encrypted ID + symmetric key (" << encrypted_key_size << " bytes)\n";
    ...
    std::cout << "[Sender] Verifying ID + symmetric key signature...\n";
    if (!verifier.verify_message(encrypted_key, signature)) {
        std::cout << "[Sender] RESULT: Signature verification failed!\n";
        std::cout << "\n";
        return kNOT_OK;
    }
    else {
    ...
    // Decrypt the ID + symmetric key using RSA private key (Bootan::secure_vector)
    Botan::AutoSeeded_RNG rng;
    ...
    // Print decrypted ID + symmetric key in hex
    std::cout << "[Sender] Decrypted ID + symmetric key (" << symmetric_key_secure.size() << " bytes)\n";
    print_botan_secure_hex(symmetric_key_secure);
    std::cout << "\n";

    // Check if the ID matches the expected receiver ID
    std::cout << "[Sender] Checking receiver ID...\n";
    if (symmetric_key_secure[0] != static_cast<uint8_t>(kIdReceiver)) {
        std::cout << "[Sender] Received receiver ID does not match expected receiver ID\n";
        return kNOT_OK;
    }

    std::cout << "[Sender] Receiver ID matches expected ID\n";
    std::cout << "\n";

    // Remove the prepended ID byte before returning the symmetric key
    symmetric_key_secure.erase(symmetric_key_secure.begin());
...
int main() {
  ...
  std::cout << "[Sender] Generating RSA key pair...\n";
  std::cout << "\n";
  Botan::AutoSeeded_RNG rng;
  ...
  switch(message_id) {
      case kMessageIdRsaPublicKey:
          std::cout << "[Sender] Send public key for session (used to encrypt symmetric key generated by Receiver)\n";
          status = send_public_key(mq_sender_to_receiver, public_key);
          message_id = kMessageIdSymKey;
          [[fallthrough]];
      case kMessageIdSymKey:
          std::cout << "[Sender] Wait for symmetric session key...\n";
          std::cout << "\n";
